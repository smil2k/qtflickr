.import "ajaxmee.js" as AJAX
.import FlickrHelper 1.0 as FlickrHelper
...
function Flickr(apiKey, apiSecret) {
    ...
    function createMethod(method, required) {
        ...
        var methodWrapper = function(args, callback) {
            ...
            args["oauth_signature"] = calculateHMAC(method, settings.url, settings.data, apiKey, apiSecret);
...
function calculateHMAC( verb, url, query, apiKey, apiSecret ) {
    var hmacKey = apiKey + "&" + apiSecret;
    var sbs = verb+"&"+encodeURIComponent(url)+"&"+encodeURIComponent(query);

    console.log(hmacKey);
    console.log(sbs);
    console.log(query);
    return FlickrHelper.CryptoHelper.hmacSha1Base64(hmacKey, sbs);
}

function _formQueryString(queryArguments) {
    var args = [],
            append = function(key) {
                args.push(key + "=" + _encodeRFC5987ValueChars(queryArguments[key]));
            };
